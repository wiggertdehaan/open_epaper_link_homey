...
function report(homey) {
  ...
  return { files: files.length, bytes, orphans, orphanBytes, byDir, paired: keep.size };
}
 ...
 * @returns {{deleted:number,bytes:number,kept:number,failed:number,files:string[]}}
 */
function cleanup(homey, options = {}) {
  ...
  const result = { deleted: 0, bytes: 0, kept: 0, failed: 0, files: [] };

  for (const f of files) {
    const owned = keep.has(f.mac.toUpperCase());
    const fresh = now - f.mtimeMs < MIN_AGE_MS;
    if (owned || fresh) { result.kept++; continue; }
