/*
 * Vendored verbatim from OpenEPaperLink, ESP32_AP-Flasher/wwwroot/g5decoder.js
 *   https://github.com/OpenEPaperLink/OpenEPaperLink
 *
 * This is the AP's own G5 decoder - the same code its web UI uses to draw tag
 * previews - so it is by construction the right decoder for the buffers the AP
 * produces. G5 is Larry Bank's format; despite the name it is NOT bitstream
 * compatible with CCITT G4, which is why a G4 decoder cannot read these files.
 *
 * LICENSING: the upstream file is under the Business Source License (see the
 * notice below), converting to Apache 2.0 on the change date. That is
 * source-available rather than open source. Review this before publishing the
 * app to the Homey App Store.
 *
 * The only change from upstream is the CommonJS export appended at the end.
 */
...
// --- added for vendoring: expose the entry point to CommonJS consumers ------
module.exports = { processG5 };
