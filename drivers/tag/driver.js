...
const { modelName } = require('../../lib/tagModels');
const { fetchAllTags } = require('../../lib/apClient');

/**
 * The one driver used to pair any OpenEPaperLink tag.
 *
 * The AP already knows the hardware type of every tag it has seen, so there is
 * no reason to make the user pick a model from a list of a hundred. This lists
 * everything /get_db returns, works out the hwType per tag, and hands Homey a
 * multi-select list. The per-model drivers still exist but are deprecated, so
 * devices paired with them keep working while they no longer clutter Add
 * Device.
 */
class TagDriver extends Driver {
   ...
   * added twice - including the ones sitting on the old per-model drivers.
   */
  pairedMacs() {
    const macs = new Set();
  ...
  /** Model name straight from the AP when it has one, our table otherwise. */
  async resolveModelName(hwType) {
    ...
    return modelName(hwType);
  }

  /** "3.0 V · 21 °C · seen 4 minutes ago" - enough to recognise a tag by. */
  describe(tag) {
    const parts = [];
  ...
  async onPairListDevices() {
    ...
    for (const tag of available) {
      ...
      devices.push({
        ...
        data: {
          // Same shape the per-model drivers used, so nothing downstream
          // needs to care which driver a device came from.
          id: tag.mac,
        },
        store: {
          hwType,
